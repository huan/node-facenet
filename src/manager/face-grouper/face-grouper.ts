...
export class FaceGrouper {
      ...
      children: (self: any) => {
        ...
        try {
          ...
          for (const child of children) {
            ...
            log.silly('FaceGrouper', 'createExplorerData() children() for(child:%s)', completePath)
          ...
          log.error('FaceGrouper', 'createExplorerData() exception: %s', e)
          // fail safe
        }
        return result
      },
  ...
  private bindSelectAction (tree: any) {
    ...
    tree.on('select', async (node: any) => {
      ...
      try {
        ...
        log.error('FaceGrouper', 'bindSelectAction() tree on select exception: %s', e)
      }
    })
  }
  ...
  public async process (file: string): Promise<void> {
    ...
    log.silly('FaceGrouper', 'process() faceList.length:%d', faceList.length)
